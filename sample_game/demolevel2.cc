...
public:
	...
	Seizure(){
		sound = Sound("sample/sounds/ding.wav");
	}

	void init()
	{
		registerDrawEvent(&Seizure::draw, 10);
	}
	...
	static void draw(Object *me)
	{
		...
		for(int i = 0; i < 4; i++)
		{
			for(int j = 0; j < 4; j++)
			{
				...
				int g = rand() * 255;
				int b = rand() * 255;
				drawRectangle(BoundingBox(i * 100, (i + 1) * 100, j * 100, (j + 1) * 100), Color(r, g, b), false);
			}
		}
		...
		p = p + v;
		drawRectangle(BoundingBox(p.x - 50, p.x + 50, p.y - 50, p.y + 50), Color(255, 0, 0, 200), false);
		drawRectangle(BoundingBox(p.x - 50, p.x + 50, p.y - 50, p.y + 50), Color(255, 255, 255, 200), true);
...
void DemoLevel2::init()
{
	addObject(new Seizure());
}
