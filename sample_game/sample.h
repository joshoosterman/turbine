...
public:
	...
	static void draw(Object *me)
	{
		...
		if (self->life > 0) {
			...
			drawLine(self->origin, end, 0xFF0000FF);
			Point tip1 = end - self->dir.rotate(30) * 5;
			drawLine(end, tip1, 0xFF0000FF);
			Point tip2 = end - self->dir.rotate(-30) * 5;
			drawLine(end, tip2, 0xFF0000FF);
			self->life--;
		} else {
			self->destroy();
		}
	...
	void init() {
		registerDrawEvent(&Arrow::draw, 40);
	}

	~Arrow() {
		unregisterDrawEvent(&Arrow::draw, 40);
	}
};
...
public:
	...
	{ 
		...
		drawRectangle(self->getBoundingBox(), 0xFF000000);
	}
	...
	static void collide(GameObject *me, GameObject *other, Vector normal) {
		...
		while (self->collides(other)) {
			self->location += escape;
		}

		self->velocity = normal * 2 * (self->velocity.negative().dotProduct(normal)) + self->velocity;
	...
	void init() {
		...
		registerMouseEvent(&Pinball::click, Input::MouseButtonPressed);
	}
};
