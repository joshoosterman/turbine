...
City *LevelLoader_create(std::string dir, int &width, int &height) {
  ...
  if (!metaFile.is_open()) {
    throw turbine::Exception("Corrupt or missing level files");
  }
...
void LevelLoader_loadTerrain(std::string dir, City *city, int width,
                             int height) {
  ...
  city->addObject(new turbine::extras::TerrainControl(
      width, height, 128, "images/terrain", terrain));
  delete terrain;
}
...
void LevelLoader_loadObjects(std::string dir, City *city) {
  ...
  while (!objectsFile.eof()) {
    ...
    city->addObject(new Building(turbine::geom::Point(x, y)));
  }
...
City *LevelLoader::load(std::string name) {
  std::string dir =
      turbine::Game::getInstance().getResourcePath("levels") + "/" + name + "/";
