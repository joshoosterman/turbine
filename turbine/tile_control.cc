...
TileControl::TileControl(int tilesX, int tilesY, int tileSize,
                         std::string contentDir, Tile *tiles)
    : SimpleGameObject(30) {
  if (tilesX == 0 || tilesY == 0 || tileSize == 0) {
    throw Exception(
        "Bad parameters to TileControl - tile size cannot be zero!");
  }
  ...
  if (size < 0) {
    throw Exception(
        "Bad parameters to TileControl - dimensions cause int overflow!");
  }
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (images.find(t.key) == images.end()) {
      ...
      graphics::Image *image = new graphics::Image(s);
      image->setOrigin(geom::Point(tileSize / 2, tileSize / 2));
...
TileControl::~TileControl() {
  // TODO(joosterman): DELETE EACH IMAGE (geom::Pointer)
  delete tiles;
}
...
Tile TileControl::getTilePosition(geom::Point p) {
  int x2 = static_cast<int>(p.x / tileSize);
  int y2 = static_cast<int>(p.y / tileSize);
...
void TileControl::draw() {
  ...
  geom::Point p1 = geom::Point(0, 0) + view->getPosition();
  geom::Point p2 = p1 + view->getSize();

  int leftX = static_cast<int>(p1.x / tileSize - 1);
  int rightX = static_cast<int>(p2.x / tileSize + 1);
  int topY = static_cast<int>(p1.y / tileSize - 1);
  int bottomY = static_cast<int>(p2.y / tileSize + 1);

  for (int i = leftX; i < rightX; i++) {
    for (int j = topY; j < bottomY; j++) {
      Tile t = getTile(i, j);
      if (t.id >= 0) {
        float xC = static_cast<float>(i + 0.5f) * tileSize;
        float yC = static_cast<float>(j + 0.5f) * tileSize;
        images[t.key]->draw(geom::Point(xC, yC));
      }
    }
  }
}
...
TerrainControl::TerrainControl(int terrainX, int terrainY, int terrainSize,
    ...
    if (tileControls.find(t) == tileControls.end()) {
      ...
      for (int j = 0; j < size; j++) {
        if (terrain[j] == t) {
          ...
          // Lower type of terrain, needs to be calculated
          tls[j] = Tile(TERRAIN_TO_CALCULATE, 0);
        } else {
          // Higher type of terrain, will be ocluded by another layer
      ...
      TileControl *tc =
          new TileControl(terrainX, terrainY, terrainSize, tcdir, tls);
      tileControls[t] = tc;
      delete tls;
    }
  }
...
int TerrainControl::getTerrainPosition(geom::Point p) {
  int x2 = static_cast<int>(p.x / terrainSize);
  int y2 = static_cast<int>(p.y / terrainSize);
