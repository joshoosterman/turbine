...
/**
Takes four parameters - the x co-ordinates for the left and right edges, and the y co-ordinates for the top and bottom.
*/
...
/// Find the BoundingBox that encompasses all points contained by both this box and and the specified other box (equivalent to the set intersection method).
/**
\param other	The BoundingBox to intersect with.
*/
BoundingBox BoundingBox::intersect(const BoundingBox& other) {
	return BoundingBox(max(left,other.left),min(right,other.right),max(top,other.top),min(bottom,other.bottom));
}
/// Returns the smallest possible BoundingBox that contains both this BoundingBox and the specified other box.
/**
\param other	The BoundingBox to include.
*/
BoundingBox BoundingBox::expandToInclude(const BoundingBox& other) {
	return BoundingBox(min(left,other.left),max(right,other.right),min(top,other.top),max(bottom,other.bottom));
}
/// Returns the smallest possible BoundingBox that contains both this BoundingBox and the specified point.
/**
\param other	The point to include.
*/
BoundingBox BoundingBox::expandToInclude(const Point& p) {
	return BoundingBox(min(left,p.x),max(right,p.x),min(top,p.y),max(bottom,p.y));
}
...
/**
In other words, this translates the BoundingBox by the position vector (rooted at [0,0]) of the given point.
*/
...
/// The main constructor for Vector. Takes x and y co-ordinates specifying the vector.
...
/// Returns the normalised version of this vector (i.e. a vector of magnitude 1 with the same direction).
Vector Vector::unit()
{
